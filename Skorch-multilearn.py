...
scaler2.fit(y_train)   # 多任务学习中，把y也标准化，消除量纲的影响
...
class MultiTaskRegressorModule(nn.Module):
    def __init__(self, input_size=17, num_units=20, output_size=4):
        ...
        self.module = nn.Sequential(  # 快速搭建法Sequential
            ...
            nn.ReLU(),  # 激励函数,当隐藏层有多层时，激励函数的影响就没有那么大
...
net = NeuralNetRegressor(
    ...
    criterion=MultiOutputMSELoss([10, 10, 10, 10]),  # 可以换成nn.MSEloss, 可以更改权重, 增加某个任务的权重会提升其性能
    max_epochs=1000,  # 训练次数,100次和1000次差别不大
...
'''
# 使用基于scikit-opt的进化算法对最佳权重进行求解
# https://scikit-opt.github.io/scikit-opt/#/zh/README
# 可以更换为差分进化算法、遗传算法、粒子群算法、模拟退火算法、蚁群算法、免疫优化算法、人工鱼群算法
def schaffer(weights):
    x1, x2, x3, x4 = weights
    net = NeuralNetRegressor(
        MultiTaskRegressorModule,
        criterion=MultiOutputMSELoss([x1, x2, x3, x4]),  # 可以换成nn.MSEloss
        max_epochs=100,  # 训练次数
        optimizer=optim.Adam,  # 优化器, Adam是个非常快的优化器
        optimizer__lr=.01,  # 学习率
    )
    ...
    result = MSE(y_train, y_pred)  # 遗传算法默认求小
    return result
...
set_run_mode(schaffer, 'parallel')  # 加速策略：并行化
...
import joblib
# save
joblib.dump(net, 'save/mtl-ga.pkl')  # 更改文件名
# restore
...
y_test = scaler2.inverse_transform(y_test)  # 反标准化
...
print("MSE of DNN on training data: %.4f" % y_train_mse)  # 未反标准化
print("r^2 of DNN on test data: %.4f" % DNN_test_score)  # 即4个任务的R2平均值
